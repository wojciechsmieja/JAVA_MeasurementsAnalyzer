...
public class FindingMinMethodTest {
    
    static Stream<String> correctTypesForMin(){
        return Stream.of("temperatures", "pressures");
    }
    /**
     * Test checks if method {@link Measurement#findMin(String)} 
     * work for correct arguments
     * 
     * Working:
     * Test add one measurement each to temperature list and pressure list 
     * then invokes method findMin with correct data types
     * 
     * Expected results:
     ...
     * @param type - Type of data for which the min is to be calculated
     * @throws MeasurementException for unknown reasons
     */
    @ParameterizedTest
    @MethodSource("correctTypesForMin")
    public void CalculateMinWithCorrectTypesNotEmptyTest(String type){
        ...
        try{
            ...
            _measurement.findMin(type);
        }catch(MeasurementException e){
            fail("Something unexpected happened while calculating average values");
            //assertEquals("Cannot calculate minimum "+type+" value",e.getMessage());
        }
    }
//---------------------------------------------------------------------------------
    static Stream<String> incorrectTypesForMin(){
     ...
     * Working:
     * Test add one measurement each to temperature list and pressure list 
     * then invokes method findMin with incorrect data types
     * 
     * Expected results:
     ...
     */   
    @ParameterizedTest
    @MethodSource("incorrectTypesForMin")
    public void CalculateMinWithIncorrectTypesNotEmptyTest(String type){
        Measurement _measurement = new Measurement();
        try{
            _measurement.addTemperatureMeasurement(LocalDate.now(), 0.0);
            _measurement.addPressureMeasurement(LocalDate.now(), 999);
...
//----------------------------------------------------------------------------------------------
    ...
    @ParameterizedTest
    @MethodSource("correctTypesForMin")
    public void CalculateMinWithCorrectTypesButEmptyTest(String type){
        Measurement _measurement = new Measurement();
        try{
            _measurement.findMin(type);
...
//------------------------------------------------------------------------------------------------
    static Stream<Arguments> correctValuesForMinMethod() {
        return Stream.of(
            Arguments.of(
     ...
     */  
    @ParameterizedTest
    @MethodSource("correctValuesForMinMethod")
    public void CalculateMinAfterAddingMeasurementsTemperatureTest(LocalDate date1, double value1, LocalDate date2, double value2,LocalDate date3, double value3) {
        try {
            Measurement _measurement = new Measurement();
            ...
            double minTemp = _measurement.findMin("temperatures");
            assertEquals(20.0, minTemp, "The minimum temperature should be 20.0");
        } catch (MeasurementException e) {
...
//------------------------------------------------------------------------------------------------------
    ...
    @ParameterizedTest
    @MethodSource("correctValuesForMinMethod")
    public void CalculateMinAfterAddingMeasurementsPressureTest(LocalDate date1, double value1, LocalDate date2, double value2,LocalDate date3, double value3) {
        try {
            Measurement _measurement = new Measurement();
            ...
            double minTemp = _measurement.findMin("pressures");
            assertEquals(20.0, minTemp, "The minimum pressure should be 20.0");
        } catch (MeasurementException e) {
            fail("Unexpected exception during integration test");
        }
    }
