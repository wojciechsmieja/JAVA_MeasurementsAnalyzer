...
public class FindingMedianMethodTest {
    
    static Stream<String> correctTypesForMedian(){
        return Stream.of("temperatures", "pressures");
    }
    /**
     * Test checks if method {@link Measurement#findMedian(String)} 
     * work for correct arguments
     * 
     * Working:
     * Test add one measurement each to temperature list and pressure list 
     * then invokes method findMedian with correct data types
     * 
     * Expected results:
     ...
     * @param type - Type of data for which the median is to be calculated
     * @throws MeasurementException for unknown reasons
     */
    @ParameterizedTest
    @MethodSource("correctTypesForMedian")
    public void CalculateMedianWithCorrectTypesNotEmptyTest(String type){
        ...
        try{
            ...
            _measurement.findMedian(type);
        }catch(MeasurementException e){
            assertEquals("Cannot calculate median "+type+" value",e.getMessage());
            fail("Something unexpected happened while calculating median values");
        }
    }
//---------------------------------------------------------------------------------
    static Stream<String> incorrectTypesForMedian(){
     ...
     * Working:
     * Test add one measurement each to temperature list and pressure list 
     * then invokes method findMedian with incorrect data types
     * 
     * Expected results:
     ...
     */    
    @ParameterizedTest
    @MethodSource("incorrectTypesForMedian")
    public void CalculateMedianWithINcorrectTypesNotEmptyTest(String type){
        Measurement _measurement = new Measurement();
        try{
            _measurement.addTemperatureMeasurement(LocalDate.now(), 0.0);
            _measurement.addPressureMeasurement(LocalDate.now(), 999);
...
//----------------------------------------------------------------------------------------------
    ...
    @ParameterizedTest
    @MethodSource("correctTypesForMedian")
    public void CalculateMedianWithCorrectTypesButEmptyTest(String type){
        Measurement _measurement = new Measurement();
        try{
            _measurement.findMedian(type);
...
//------------------------------------------------------------------------------------------------
    static Stream<Arguments> correctValuesForMedianMethod() {
        return Stream.of(
            Arguments.of(
                LocalDate.of(2020, 10, 11), 25.5,
     ...
     */    
    @ParameterizedTest
    @MethodSource("correctValuesForMedianMethod")
    public void CalculateMedianAfterAddingMeasurementsTemperatureTest(LocalDate date1, double value1, LocalDate date2, double value2,LocalDate date3, double value3) {
        try {
            Measurement _measurement = new Measurement();
            ...
            double medianTemp = _measurement.findMedian("temperatures");
            assertEquals(25.5, medianTemp, "The minimum temperature should be 25.5");
        } catch (MeasurementException e) {
...
//------------------------------------------------------------------------------------------------------
    ...
    @ParameterizedTest
    @MethodSource("correctValuesForMedianMethod")
    public void CalculateMedianAfterAddingMeasurementsPressureTest(LocalDate date1, double value1, LocalDate date2, double value2,LocalDate date3, double value3) {
        try {
            Measurement _measurement = new Measurement();
            ...
            double medianTemp = _measurement.findMedian("pressures");
            assertEquals(25.5, medianTemp, "The minimum pressure should be 25.5");
        } catch (MeasurementException e) {
            fail("Unexpected exception during integration test");
        }
    }
