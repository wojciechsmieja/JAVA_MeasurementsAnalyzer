...
public class FindingMaxMethodTest {
    
    static Stream<String> correctTypesForMax(){
        return Stream.of("temperatures", "pressures");
    }
    /**
     * Test checks if method {@link Measurement#findMax(String)} work for correct arguments
     * 
     * Working:
     * Test add one measurement each to temperature list and pressure list 
     * then invokes method findMax with correct data types
     * 
     * Expected results:
     ...
     * @param type - Type of data for which the max is to be calculated
     * @throws MeasurementException for unknown reasons
     */    
    @ParameterizedTest
    @MethodSource("correctTypesForMax")
    public void CalculateMaxWithCorrectTypesNotEmptyTest(String type){
        ...
        try{
            ...
            _measurement.findMax(type);
        }catch(MeasurementException e){
            assertEquals("Cannot calculate maximum "+type+" value",e.getMessage());
            fail("Something unexpected happened while calculating average values");
        }
    }
//---------------------------------------------------------------------------------
    static Stream<String> incorrectTypesForMax(){
        return Stream.of("temperature", "nothing");
    }
    /**
     * Test checks if method {@link Measurement#findMax(String)} 
     * work for incorrect arguments
     * 
     * Working:
     * Test add one measurement each to temperature list and pressure list 
     * then invokes method findMax with incorrect data types
     * 
     * Expected results:
     ...
     */    
    @ParameterizedTest
    @MethodSource("incorrectTypesForMax")
    public void CalculateMaxWithINcorrectTypesNotEmptyTest(String type){
        Measurement _measurement = new Measurement();
        try{
            _measurement.addTemperatureMeasurement(LocalDate.now(), 0.0);
            _measurement.addPressureMeasurement(LocalDate.now(), 999);
...
//----------------------------------------------------------------------------------------------
    ...
    @ParameterizedTest
    @MethodSource("correctTypesForMax")
    public void CalculateMaxWithCorrectTypesButEmptyTest(String type){
        Measurement _measurement = new Measurement();
        try{
            _measurement.findMax(type);
...
//------------------------------------------------------------------------------------------------
    static Stream<Arguments> correctValuesForMaxMethod() {
        return Stream.of(
            Arguments.of(
     ...
     */ 
    @ParameterizedTest
    @MethodSource("correctValuesForMaxMethod")
    public void CalculateMaxAfterAddingMeasurementsTemperatureTest(LocalDate date1, double value1, LocalDate date2, double value2,LocalDate date3, double value3) {
        try {
            Measurement _measurement = new Measurement();
            ...
            double maxTemp = _measurement.findMax("temperatures");
            assertEquals(30.0, maxTemp, "The minimum temperature should be 30.0");
        } catch (MeasurementException e) {
...
//------------------------------------------------------------------------------------------------------
    ...
    @ParameterizedTest
    @MethodSource("correctValuesForMaxMethod")
    public void CalculateMaxAfterAddingMeasurementsPressureTest(LocalDate date1, double value1, LocalDate date2, double value2,LocalDate date3, double value3) {
        try {
            Measurement _measurement = new Measurement();
            ...
            double maxTemp = _measurement.findMax("pressures");
            assertEquals(30.0, maxTemp, "The minimum pressure should be 30.0");
        } catch (MeasurementException e) {
            fail("Unexpected exception during integration test");
        }
    }
